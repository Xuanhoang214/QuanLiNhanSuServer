...
router.get('/ToanBoCongTrinh', async (req,res)=>{
    ...
    if(_isCurrent > 0)
    {
      var _result = await client.db('QUANLYNHANSU').collection('CongTrinh').find({}).toArray();
...
router.post('/ThemCongTrinh',async (req,res)=>{
   // Encode.DecryptionCode("abkjbuguguc","123");
    ...
    if(_isCurrent > 0)
    {
      var _result = await client.db('QUANLYNHANSU').collection('CongTrinh').find({TenCT:req.body.TenCT}).count();
      ...
      {
         await client.db('QUANLYNHANSU').collection('CongTrinh').insertOne(req.body);
...
router.post('/CapNhatCongTrinh',async (req,res)=>{
   ...
   var _isCurrent = await client.db('QUANLYNHANSU').collection('IDSession').find({token:req.query.token}).count();
      ...
      if(_isCurrent > 0)
      {
         var _result = await client.db('QUANLYNHANSU').collection('CongTrinh').updateOne({MaCT:req.body.MaCT},{$set:
            {
         ...
         {
            res.send({success:true,mess:'Cập Nhật Thành Công',data:req.body})
         }
      }else
      {
            res.send({success:false,mess:"Mã Xác Thực Không Tồn Tại !"});
...
router.delete('/XoaCongTrinh',async (req,res)=>{
   ...
   if(_isCurrent > 0)
   {
      ...
      }else
      {
         ...
         {
            var _result = await client.db('QUANLYNHANSU').collection('CongTrinh').find({MaCT:req.body.MaCT}).count();
            if(_result == 1)
            {
               await client.db('QUANLYNHANSU').collection('CongTrinh').deleteOne({MaCT:req.body.MaCT})
