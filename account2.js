...
router.post('/login',async (req,res)=>{
    console.log("connect");
    var _result = await client.db('QUANLYNHANSU').collection('NhanVien2').find({SDT:req.body.SDT,Pass:req.body.Pass}).count();
    if(_result > 0)
    {
        var _data = await client.db('QUANLYNHANSU').collection('NhanVien2').find({SDT:req.body.SDT,Pass:req.body.Pass}).toArray();
...
router.get('/allNhanVien',async (req,res)=>{
    ...
    if(_isCurrent > 0 )
    {
        var _result = await client.db('QUANLYNHANSU').collection('NhanVien2').find({}).toArray();
...
router.post('/CapNhatThongTin',async (req,res)=>{
    ...
    if(_isCurrent > 0 )
    {
        await client.db('QUANLYNHANSU').collection('NhanVien2').updateOne({MaNV:req.body.MaNV},
            {$set:
            {
...
router.post('/create', async (req,res)=>{
    ...
    var _account = await client.db('QUANLYNHANSU').collection('NhanVien2').find({SDT:req.body.SDT}).count();
    if(_account > 0)
    {
        res.send({success:false,mess:"Tài Khoản Đã Tồn Tại !"});
        return;
    }
    var _result = await client.db('QUANLYNHANSU').collection('NhanVien2').insertOne(req.body);
